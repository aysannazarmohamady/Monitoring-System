...
// ===================== ارزیابی: ابر کلمات کلیدی تیترها =====================

// فهرست حروف اضافه، کلمات ربط و کلمات پرتکرار بی‌بار معنایی فارسی که باید از ابر کلمات حذف شوند
function persianStopwords(): array
{
    static $list = null;
    if ($list !== null) return $list;
    $words = [
        'از','به','با','در','برای','تا','بر','بی','بدون','روی','زیر','کنار','میان','بین','نزد','سوی','طی','ضمن','پیش','پس','همراه',
        'که','و','یا','اما','ولی','اگر','چون','زیرا','چرا','همچنین','نیز','هم','نه','یعنی','مانند','مثل','چگونه','چطور',
        'را','این','آن','آنها','اینها','او','ایشان','ما','شما','آنان','اینان','خود','خویش',
        'های','ها','ای','یی','است','هست','هستم','هستی','هستیم','هستید','هستند','نیست','نیستم','نیستی','نیستیم','نیستید','نیستند',
        'بود','بودم','بودی','بودیم','بودید','بودند','باشد','باشیم','باشند','باشم','باشی',
        'شد','شدم','شدی','شدیم','شدید','شدند','شده','شده‌اند','شوند','شود','شویم','شوم','شوی','بشود','نشود','می‌شود','میشود','می‌شوند','میشوند',
        'کرد','کردم','کردی','کردیم','کردید','کردند','کرده','کرده‌اند','کنند','کند','کنیم','کنم','کنی','کنید','بکند','نکند','می‌کند','میکند','می‌کنند','میکنند','می‌کنیم','میکنیم',
        'دارد','دارم','داری','داریم','دارید','دارند','داشت','داشتم','داشتی','داشتیم','داشتید','داشتند','داشته',
        'خواهد','خواهم','خواهی','خواهیم','خواهید','خواهند','باید','نباید','شاید',
        'رفت','رفتم','رفتی','رفتیم','رفتید','رفتند','رود','روند','برود','می‌رود','میرود','می‌روند','میروند',
        'آمد','آمدم','آمدی','آمدیم','آمدید','آمدند','آید','آیند','بیاید','می‌آید','میاید','می‌آیند','میایند',
        'داد','دادم','دادی','دادیم','دادید','دادند','دهد','دهند','بدهد','می‌دهد','میدهد','می‌دهند','میدهند',
        'گفت','گفتم','گفتی','گفتیم','گفتید','گفتند','گوید','گویند','بگوید','می‌گوید','میگوید','می‌گویند','میگویند',
        'گرفت','گرفتم','گرفتی','گرفتیم','گرفتید','گرفتند','گیرد','گیرند','بگیرد','می‌گیرد','میگیرد','می‌گیرند','میگیرند',
        'توانست','توانستم','توانستی','توانستیم','توانستید','توانستند','تواند','توانند','می‌تواند','میتواند','می‌توانند','میتوانند','می‌توان','میتوان',
        'یک','دو','سه','چند','همه','هر','هیچ','بعضی','برخی','دیگر','دیگری','سایر','همان','همین','چنین','آنچه','هرچه',
        'را','اش','ام','ات','مان','تان','شان',
        'اکنون','امروز','دیروز','فردا','هنوز','دوباره','اینک','سپس','آنگاه','بنابراین','درنتیجه','بلکه','گرچه','هرچند','ضمناً','البته','لطفا','لطفاً',
    ];
    $list = [];
    foreach ($words as $w) { $list[normalizePersianChars($w)] = true; }
    return $list;
}

// یک تیتر را به فهرست توکن‌های اصلی (بدون نشانه‌گذاری) تبدیل می‌کند
function tokenizeTitleWords(string $title): array
{
    $t = str_replace(['ي', 'ك'], ['ی', 'ک'], $title);
    $t = preg_replace('/[\x{200C}\x{200F}\x{200E}]/u', ' ', $t);
...
// از یک تیتر، عبارات ۱ تا ۴ کلمه‌ای (که با حرف اضافه/ربط شروع یا پایان نیابند) استخراج می‌کند
function extractTitleNgrams(string $title, array $stopwords, int $maxN = 4): array
{
    $tokens = tokenizeTitleWords($title);
    $n = count($tokens);
    if ($n === 0) return [];
    $out = [];
    for ($size = 1; $size <= $maxN; $size++) {
        for ($i = 0; $i + $size <= $n; $i++) {
            ...
            $first = normalizePersianChars($slice[0]);
            $last = normalizePersianChars($slice[$size - 1]);
            if (mb_strlen($first) < 3 || isset($stopwords[$first])) continue;
            if ($size > 1 && (mb_strlen($last) < 3 || isset($stopwords[$last]))) continue;
            if (preg_match('/^[0-9۰-۹]+$/u', $first)) continue;
            $phrase = implode(' ', $slice);
            $key = normalizePersianChars($phrase);
            $out[] = ['key' => $key, 'text' => $phrase];
        }
    }
    return $out;
}

// ابر کلمات کلیدی تیترها را از مجموعه‌ای از ردیف‌های اخبار می‌سازد (پس از اعمال فیلترهای پایه/پیشرفته روی $rows)
function buildTitleWordCloud(array $rows, int $limit = 60, int $maxN = 4): array
{
    $stopwords = persianStopwords();
    $counts = []; // key => count
    $display = []; // key => text (رایج‌ترین شکل نمایشی)
    $displayCounts = []; // key => [text => count] برای انتخاب پرتکرارترین شکل نمایشی
    foreach ($rows as $r) {
        $title = trim((string)($r['title'] ?? ''));
        if ($title === '') continue;
        foreach (extractTitleNgrams($title, $stopwords, $maxN) as $ng) {
            $counts[$ng['key']] = ($counts[$ng['key']] ?? 0) + 1;
            $displayCounts[$ng['key']][$ng['text']] = ($displayCounts[$ng['key']][$ng['text']] ?? 0) + 1;
        }
    }
    arsort($counts);
    $out = [];
    foreach ($counts as $key => $cnt) {
        if ($cnt < 2) continue; // کلمات یک‌باره برای ابر کلمات مفید نیستند
        $texts = $displayCounts[$key];
        arsort($texts);
        $text = array_key_first($texts);
        $out[] = ['text' => $text, 'count' => $cnt, 'words' => count(explode(' ', $text))];
